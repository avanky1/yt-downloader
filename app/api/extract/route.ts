...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { stdout: infoOut } = await execAsync(
      `yt-dlp --cookies /home/ubuntu/yt-downloader/cookies.txt --no-warnings --dump-json "${url}"`
    );
    ...
    if (mp4Formats.length === 0) {
      const { stdout } = await execAsync(
        `yt-dlp --cookies /home/ubuntu/yt-downloader/cookies.txt --no-warnings --get-url -f "${formatId}" "${url}"`
      );
